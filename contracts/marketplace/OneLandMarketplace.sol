...
contract OneLandMarketplace is
    Initializable,
    IMarketplace,
    ...
    IERC1155ReceiverUpgradeable
{
    /*///////////////////////////////////////////////////////////////
                            State variables
    //////////////////////////////////////////////////////////////*/
    // address public projectOwner = msg.sender;
    ...
    // // 函数用于授予 LISTER_ROLE 角色给其他地址
    // function grantListerRole(address account) public {
    //     require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");
    //     grantRole(LISTER_ROLE, account);
    // }

    // // 函数用于授予 ASSET_ROLE 角色给其他地址
    // function grantAssetRole(address account) public {
    //     require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");
    //     grantRole(ASSET_ROLE, account);
    // }
    ...
    function initialize(
        ...
        // _setupRole(ASSET_ROLE, address(0));
        // _setupRole(ASSET_ROLE, msg.sender);
    }
    ...
    function buy(
        uint256 _listingId, // id
        address _buyFor, // 最终买家地址
        uint256 _quantityToBuy, // 购买数量
        address _currency, // 代币地址
        uint256 _totalPrice // 总价格
    ) external payable override nonReentrant onlyExistingListing(_listingId) {
        Listing memory targetListing = listings[_listingId];
        address payer = _msgSender(); // 调用者
        ...
        executeSale(
            targetListing, // listings
            payer, // 调用者
            _buyFor, // 最终买家地址
            targetListing.currency, // 代币地址
            targetListing.buyoutPricePerToken * _quantityToBuy, // 总价格
            _quantityToBuy // 购买数量
        );
    }
    ...
    function executeSale(
        ...
        address _currency, // 代币地址
        uint256 _currencyAmountToTransfer, // 总价格
        uint256 _listingTokenAmountToTransfer // 购买数量
    ) internal {
        ...
        emit NewSale(
            _targetListing.listingId, // listingId
            _targetListing.assetContract, // 资产合约
            _targetListing.tokenOwner, // 资产创建者
            _receiver, // 最终买家地址
            _listingTokenAmountToTransfer, //购买数量
    ...
    function validateDirectListingSale(
        ...
        if (_currency == CurrencyTransferLib.NATIVE_TOKEN) {
            ...
            validateERC20BalAndAllowance(_payer, _currency, settledTotalPrice); // 调用者， 代币地址， 总价格
        }
